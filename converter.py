...
parser.add_argument('-s', '--default', type=str, help='Setting the default currency', default='UAH')
parser.add_argument('-c', '--convert', help='Performs converting the currencies', action="store_true")
parser.add_argument('-hi', '--history', help='Show history of the realtime exchange rate', action='store_true')
parser.add_argument('-i', '--info', help='Show information about currencies', action='store_true')
parser.add_argument('-am', type=int, help='The amount you want to convert')

args = parser.parse_args()

api_base = "https://www.alphavantage.coh/query?function="
api_key = "API_KEY"
try:
...
def convert(from_currency, to_currency, amount):
    ...
    url = "{api_base}{function}&from_currency={from_currency}&to_currency={to_currency}&apikey={api_key}".format(
        api_base=api_base,
        function="CURRENCY_EXCHANGE_RATE",
    ...
    exchange_rate = json_exc.get('Realtime Currency Exchange Rate')
    if not exchange_rate:
        return None
    exchange_rate = exchange_rate.get('5. Exchange Rate')
    if not exchange_rate:
        return None
    return round(float(exchange_rate) * amount, 3)
...
# python converter.py -c -fc uah -tc usd -am 100
